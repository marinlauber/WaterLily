...
# QNCouple = Relaxation(dⁿ(struc),f_old;relax=ωᵣ)
...
@time @gif for tᵢ in range(t₀,t₀+duration;step)
    ...
    global twindow,f_old, pnts_old, updated_values;
    ...
    while t < tᵢ*sim.L/sim.U
        ...
        while true
            ...
            if ((rd<1e-2) && (rf<1e-2)) || iter > 20 # if we converge, we exit to avoid reverting the flow
                println("  Converged...")
            ...
            writetxt("_resiuals$(twindow)_$iter", QNCouple.r)
            writetxt("_matrixV$(twindow)_$iter", QNCouple.V)
            writetxt("_matrixW$(twindow)_$iter", QNCouple.W)
            writetxt("_c$(twindow)_$iter", QNCouple.c)
